...
filename = os.environ.get('OCHRE_FILENAME', 'HPWH_AdmissionControl_n95_testx8_cap8LU')
...
Duration = 2  # days
t_res = int(os.environ.get('OCHRE_TIMESTEP_MIN', 1))  # minutes
...
# SHED = ramp-down reserve (reduce load) -- what's actually validated in
# this project. LOAD = ramp-up reserve (increase load) -- the branch
# exists in determine_reserve_control() but has never been run/validated.
# excel_ochre.py's RD-CF_event write-back assumes SHED; do not switch
# this to LOAD without also updating that assumption.
RESERVE_COMMAND = 'LOAD'

# OCHRE_DISPATCH_TIME / OCHRE_DURATION_HR override these when set (used
# by excel_ochre.py, sourced from the Excel sheet's Start/Stop cells) --
# unset, manual runs use the same 15:00/1.5hr validated baseline as before.
reserve_event = {
    'dispatch_time': os.environ.get('OCHRE_DISPATCH_TIME', '15:00'),
    'duration': float(os.environ.get('OCHRE_DURATION_HR', 1.5))   # hours
}
...
#########################################
# POWER-CAPPED ADMISSION CONTROL  -- NEW (V3)
#########################################
# Replaces the fixed, precomputed RELEASE_WINDOW_MINUTES/release_delay_minutes
# staggering (V2) with a dynamic admission scheme: units are still ranked
# warmest-first from event_end (priority order), but WHEN a unit actually
# gets released now depends on real-time fleet conditions, not a delay
# decided in advance. At each timestep after event_end, the next-highest-
# priority unit still waiting is admitted (switched to baseline setpoint/
# deadband) only if the fleet's current aggregate Water Heating Electric
# Power is already below this cap; otherwise it waits one more timestep
# and is reconsidered. Once admitted, a unit stays admitted -- the cap
# only gates new admissions, not units already recovering.
#
# Sizing note: "current aggregate power" per home ~7.5 kW fleet-wide
# during the 15:00-16:30 baseline (no-dispatch) window (n=95 homes); the
# fleet's baseline daily peak WH power is ~15 kW.
#
# A tight cap suppresses the peak more but takes longer to clear the
# fleet; a relaxed cap clears faster but reduces the peak less. Set this
# directly per test -- 15.0 kW is the value validated at n=95 (0%
# never-admitted, 180.6 min to 100%). Scaling this to a different fleet
# size requires deriving a new value from THAT fleet's own measured
# baseline power (see the n=498 run, which needed 82.2 kW) -- never
# assume a ratio, measure it.
CAP_KW = 8.0

# Per-timestep admission count -- how many next-highest-priority waiting
# units get admitted at once, when the cap allows an admission at all.
# 1/timestep is the value validated at n=95 (0% never-admitted). Raising
# it clears a larger backlog faster but reintroduces synchronized overlap
# (see the n=95 rate sweep: peak per-unit power rose from 184.7 W/unit at
# 1/timestep to 246.7 W/unit at 10/timestep) -- so don't raise it further
# than the fleet size actually requires.
...
#########################################
# RESERVE CONTROL FUNCTION -- used by Phase 1 only in V3
#########################################
...
def determine_reserve_control(sim_time, event_cfg, unit_delay_minutes=0,
    ...
    setpoint/deadband. In V3 this is only ever called by Phase 1 (with
    release_delay_minutes held far in the future) -- the post-event
...
def run_admission_controlled_recovery(phase1_results, event_cfg):
    ...
    constructed Dwelling, replacing V2's fixed-release_delay Phase 2.
    ...
    to completion in parallel threads the way V2 did. OCHRE's per-timestep
    compute itself is cheap -- Dwelling construction/schedule-loading
    (Phase 1, already parallelized) is what dominated runtime before, not
    ...
    minutes to clear a fleet of n waiting units -- for n=498 at 1/timestep
    that floor alone (~8.3 hr) was most of the remaining simulation
    window, which is what produced the 20.3% never-admitted result even
    with a correctly fleet-scaled power cap. Raise it deliberately when
...
#########################################
# MAIN EXECUTION  -- V3: Phase 1 (parallel) -> admission control (sequential) -> finalize (parallel)
#########################################
...
if __name__ == "__main__":
    ...
    # ---- NEW (V3): power-capped admission control, single synchronized
    # lockstep loop across every home (replaces V2's precomputed
    # RELEASE_WINDOW_MINUTES + independent-per-home Phase 2) ----
